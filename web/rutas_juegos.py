...
def guardar_pelicula():
    ...
    if (content_type == 'application/json'):
        ...
        ret,code=controlador_peliculas.insertar_pelicula(pelicula_json["nombre"], pelicula_json["descripcion"], float(pelicula_json["precio"]), pelicula_json["foto"])
    else:
...
def actualizar_pelicula():
    ...
    if (content_type == 'application/json'):
        pelicula_json = request.json
        ret,code=controlador_peliculas.actualizar_pelicula(pelicula_json["id"],pelicula_json["nombre"], pelicula_json["descripcion"], float(pelicula_json["precio"]),pelicula_json["foto"])
    else:
        ret={"status":"Bad request"}
        code=401
    return json.dumps(ret), code